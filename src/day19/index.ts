...
const prepareInput = (rawInput: string) => {
  ...
  parts[0].split('\n').forEach(rule => {
    ...
    const value = rules[0].includes('"') ? rules[0][1] : rules.map((c) => c.split(' ').map((n) => parseInt(n)))
...
const checkLine = (message:string, stack) => {
  ...
  if (typeof c == "string") {
    if (message[0] == c)
      return checkLine(message.slice(1), stack.slice())
  } else {
    for (const rule of rules.get(c)) {
      const newStack = typeof rule == "string" ? stack.concat([rule]) : stack.concat([...rule.slice().reverse()])
...
const goA = (input) => {
  return input.messages.filter(message => checkLine(message, [...rules.get(0)[0].slice().reverse()])).length
}
