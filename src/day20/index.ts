...
const prepareInput = (rawInput: string): Photo[] => rawInput.split('\n\n').map(x => {
...
const rotate = (photo: Photo) => {
  photo.body = [...photo.body[0]].map((val, index) => photo.body.map(row => row[index]).reverse().join(''))
}
...
const matchDirection = (grid: Map<string, Photo>, photo: Photo, cord: number[], photoDirF, matchDirF, dir: number[]) => {
...
const goA = (input: Photo[]) => {
  const oldPhotos = []
  grid.set([0, 0].toString(), input[0])

  while (oldPhotos.length < input.length) {
    const photos = [...grid.keys()].filter(x => !oldPhotos.includes(x))
    const photo = grid.get(photos[0])
  ...
  const cords = [...grid.keys()].map(x => x.split(',').map(x => parseInt(x)))
...
const joinGrid = () => {
  ...
  input.forEach(x => x.body = x.body.slice(1, -1).map(x => x.slice(1, -1)))
  let res = ""
...
const rotateImage = (image: string[]) => {
  return [...image[0]].map((val, index) => image.map(row => row[index]).reverse().join('')).join('\n')
}
...
const goB = (input: Photo[]) => {
  ...
  while (true) {
    ...
    for (let i = 0; i < tmp.length - 2; i++) {
      const l1 = [...tmp[i].matchAll(/(..................#.)/g)].map((x) => x.index)
      const l2 = [...tmp[i + 1].matchAll(/(#....##....##....###)/g)].map((x) => x.index)
      const l3 = [...tmp[i + 2].matchAll(/(.#..#..#..#..#..#...)/g)].map((x) => x.index)
